...
int main() {
    ...
    // Generate permutations of initial permutation table for testing
    cout << "Testing different initial permutations using next_permutation:\n";
    vector<int> perm = initial_perm; // Copy initial permutation table
    int count = 0;
    do {
        cout << "Permutation " << ++count << ":\n";
        string cipher_text = bin2hex(encrypt(pt, rkb, rk));
        cout << "Cipher Text: " << cipher_text << endl;
        initial_perm = perm; // Update initial_perm for encryption
        if (count >= 3) break; // Limit to 3 permutations for brevity
    } while (next_permutation(perm.begin(), perm.end()));

    cout << "Encryption with original initial permutation:\n";
    initial_perm = { // Restore original initial permutation
        58, 50, 42, 34, 26, 18, 10, 2,
        60, 52, 44, 36, 28, 20, 12, 4,
        62, 54, 46, 38, 30, 22, 14, 6,
        64, 56, 48, 40, 32, 24, 16, 8,
